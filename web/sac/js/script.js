angular.module("app").value("config", {
    urlVerNumPedido: "sac/ajax/verifica-num-pedido",
    urlVerClienteEmail: "sac/ajax/verifica-cliente-email"
});
    ...
    $scope.verificarNumeroPedido = function(numPedido) {
        ...
        if(numPedido){
            ...
            postService.postService(data,config.urlVerNumPedido).then(
                function(data){
                    if(!data.data.status){
                        $scope.blocoNumError = true;
                        $scope.msgNumError = data.data.msg;
                    }else{
